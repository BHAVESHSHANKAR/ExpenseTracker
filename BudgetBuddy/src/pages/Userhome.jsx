...
// import { User, LogOut, MessageSquare } from 'lucide-react'; // Added MessageSquare for assistant icon
...
//   // Image-based expense tracker states
...
//         `${import.meta.env.VITE_API_URL}/api/auth/expenses/gemini`,
//         { image: imageData },
...
//     try {
//       const response = await axios.post(
//         `${import.meta.env.VITE_API_URL}/api/auth/assistant`,
...
function Userhome() {
  ...
  const updateImageChart = () => {
    if (!tempExpenses || !chartRef.current) {
      toast.error('Process an image first to generate the chart.');
      return;
    }
    ...
    setChartInstance(newChart);
    toast.success('Image chart updated!');
  };
  ...
  const saveImageExpenses = async () => {
    if (!tempExpenses) {
      toast.error('Process an image first to save expenses.');
      return;
    }
    ...
    try {
      ...
      setExpensesList(response.data.expenses);
      resetImageTracker();
      toast.success('Image expenses saved! Check your email for details.');
    } catch (error) {
      console.error('Error saving image expenses:', {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data,
      });
      toast.error(error.response?.data?.message || 'Failed to save image expenses.');
    } finally {
      setIsSavingImage(false);
    }
  };
  ...
  const handleAddManualExpense = () => {
    if (!manualExpenseName || !manualExpenseValue) {
      toast.error('Please enter both an expense name and value.');
      return;
    }

    const value = parseFloat(manualExpenseValue);
    if (isNaN(value) || value <= 0) {
      toast.error('Please enter a valid positive expense value.');
      return;
    }
  ...
  const updateManualChart = () => {
    if (!Object.keys(manualExpenses).length || !manualChartRef.current) {
      toast.error('Add manual expenses first to generate the chart.');
      return;
    }
    ...
    setManualChartInstance(newChart);
    toast.success('Manual chart updated!');
  };
  ...
  const saveManualExpenses = async () => {
    if (!Object.keys(manualExpenses).length) {
      toast.error('Add manual expenses first to save.');
      return;
    }
    ...
    try {
      ...
      setExpensesList(response.data.expenses);
      resetManualTracker();
      toast.success('Manual expenses saved! Check your email for details.');
    } catch (error) {
      console.error('Error saving manual expenses:', {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data,
      });
      toast.error(error.response?.data?.message || 'Failed to save manual expenses.');
    } finally {
      setIsSavingManual(false);
    }
  };
          ...
          <div className="uh-instructions-card">
            ...
            <div className="uh-instructions-list">
              ...
              <ul>
                ...
                <li>Use "Expense Tracker" to process expense images.</li>
                <li>Use "Manual Expense Entry" to add expenses manually.</li>
                <li>Chat with your Personal Assistant.</li>
              </ul>
            </div>
          </div>
        </div>
